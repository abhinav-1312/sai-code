...
export const handleSearch = (
  ...
  if (searchText !== null) {
    ...
    if (setSearch !== null) setSearch(sanitizedText);
...
const recursiveSearch = (object, searchText) => {
  for (let key in object) {
    ...
    } else if (value && sanitizeText(value).includes(searchText)) {
      return true;
    }
  }
  return false;
...
export const printOrSaveAsPDF = async (formRef) => {
  ...
  input.style.margin = "-13rem 0 0 0"; // Adjust the negative margin as needed to shift the content upward
...
export const fetchUomLocatorMaster = async (setUomHook, setLocatorHook) => {
  try {
    ...
    const locatorObj = convertArrayToObject(
      locatorMasterData,
      "id",
      "locatorDesc"
    );
...
export const daysDifference = (issueDate, receivingDate) => {
  ...
  const receivingDateMod = new Date(
    receivingYear,
    receivingMonth - 1,
    receivingDay
  ...
  const differenceMs = Math.abs(
    receivingDateMod.getTime() - issueDateMod.getTime()
  );
